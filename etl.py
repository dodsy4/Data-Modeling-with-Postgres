...
def process_log_file(cur, filepath):
    ...
    * filepath the file path to the song file
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (index, row.ts, int(row.userId), row.level, song_id, artist_id, row.sessionId, row.location, row.userAgent)
...
def process_data(cur, conn, filepath, func):
    ...
    INPUTS:
    * cur the cursor variable
    * conn: psycopg2 curser
    * filepath the file path to the song file
    * func function for file processsing
    """
